...
const SubjectPage = (props) => {
  ...
  const { subjectsList, setClassModalVisible, classModalState, setClassModalState} = props;
      ...
      <div className={`modal-dialog ${styles.panel}`} role="document">
        <div class="modal-content h-100">
          <div className={`modal-header ${styles.modalHeader}`}>
            ...
            <h5 className={`modal-title ${styles.modalTitle}`} id="exampleModalLabel">{classModalState.major}</h5>
          ...
          <div class="modal-body overflow-auto">
            <div class="list-group">
              {subjectsList.filter(e => e.toLowerCase().includes(filterValue.toLowerCase())).map((item) => (
                <button 
                  type="button" 
